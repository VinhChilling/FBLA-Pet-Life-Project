...
function selectDifficulty(level) {
  ...
  document.getElementById("difficultyInfo").textContent = config.info;
}
...
function feedPet() {
  ...
  const food = FOOD_SHOP[player.foodTier];
...
function updatePlayerStats() {
  ...
  if (pointsEl) {
    ...
    const percentage = Math.round(
      (player.currentPoints / maxPossibleScore) * 100,
    );
